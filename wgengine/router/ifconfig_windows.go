...
// to apply SkipAsSource to prevent them from being used, a the addresses will
// not be routed on the Tailscale interface.
func syncAddresses(ifc *winipcfg.IPAdapterAddresses, want []*net.IPNet) error {
	...
	for _, a := range del {
		...
		if err != nil {
			erracc = append(erracc, fmt.Errorf("deleting IP %q: %w", *a, err))
		}
	}
	...
	for _, a := range add {
		...
		if err != nil {
			erracc = append(erracc, fmt.Errorf("adding IP %q: %w", *a, err))
		}
	}
	...
	for _, a := range ll {
		...
		if err != nil {
			erracc = append(erracc, fmt.Errorf("setting skip-as-source on IP %q: unable to retrieve MIB: %w", *a, err))
			continue
		}
		mib.SkipAsSource = true
		if err := mib.Set(); err != nil {
			erracc = append(erracc, fmt.Errorf("setting skip-as-source on IP %q: unable to set MIB: %w", *a, err))
		}
	}

	return multierr.New(erracc...)
}
